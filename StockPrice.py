...
def get_stock_data(symbol_name, year_list):
    try:
        ...
        for year in year_list:
            ...
            df = yf.download(symbol_name, start=start, end=end)
            data_frames.append(df)

        yearly_data = pd.concat(data_frames)

        # Check the type of the index before conversion
        print(f"Before conversion: {type(yearly_data.index)}")

        yearly_data.index = pd.to_datetime(yearly_data.index)  # Convert the index to datetime

        # Check the type of the index after conversion
        print(f"After conversion: {type(yearly_data.index)}")
...
def plot_stock_data(data, symbol, title, show_moving_average=True):
    ...
    # Customize the plot
    fig.update_layout(
        title=f"{title} for {symbol} Over Time",
        xaxis_title="Year",
...
def predict_stock_prices(data, symbol, years_prediction):
    ...
    # Fit ARIMA model
    model = ARIMA(closing_prices, order=(5, 1, 2))  # Example order, adjust as needed
    results = model.fit()
    ...
    # Assign the forecasted values to the DataFrame
